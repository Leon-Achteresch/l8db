...
function check() {
  ...
  if (unique.size !== 1) {
    console.error("Version mismatch between package.json, Cargo.toml and tauri.conf.json");
    process.exit(1);
  }
...
function set(version) {
  ...
  try {
    ...
    writeFileSync(new URL(lockPath, root), `${JSON.stringify(lock, null, 2)}\n`);
  } catch {
  ...
  console.log(`Version set to ${version} in package.json, package-lock.json, Cargo.toml, tauri.conf.json`);
}
