...
(() => {
  ...
  if (!localStorage.getItem("l8db.connections")) {
    localStorage.setItem("l8db.connections", JSON.stringify({state:{connections:[{id:"ch",name:"ClickHouse · Test",kind:"clickhouse",connectionString:"clickhouse://l8db:l8db@localhost:8124/shop",sslMode:"disable"}],activeId:"ch",favoriteServerKeys:[],serverOrder:[]},version:0}));
    localStorage.setItem("l8db.settings", JSON.stringify({state:{tourFinished:true},version:0}));
    localStorage.setItem("l8db.db-selection", JSON.stringify({state:{database:"shop",schema:"shop"},version:0}));
  }
})();
`;
Bun.serve({
