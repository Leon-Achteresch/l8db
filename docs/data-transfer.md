...
File imports share one backend pipeline (`db/import.rs`) for PostgreSQL, MySQL/MariaDB, SQLite, SQL Server, Oracle, DuckDB and ClickHouse. Rows are written as multi-row INSERT batches (Oracle 100, SQL Server 500, ClickHouse 5,000, otherwise 1,000 rows or 2 MiB of SQL) inside one transaction with a savepoint per batch; a failing batch is replayed row by row to report the exact row. Conflict handling maps to `ON CONFLICT`, `ON DUPLICATE KEY UPDATE` or `MERGE`. ClickHouse has no transactions or conflict handling, so a failed import leaves already written batches. MongoDB imports use `insert_many` or per-document upserts and are not transactional. Sources are CSV, JSON (array of objects, streamed), NDJSON, XLSX (sheet, skipped rows and header row; the sheet is loaded into memory, 256 MiB file limit) and Parquet (row reader). All formats share mapping, presets and a type-inference preview.

Parquet, XML and HTML exports reuse the atomic temporary-file writer. Parquet columns are typed (boolean, int64, double, date, timestamp, otherwise UTF-8) and written in row groups of 50,000 rows. Loaded rows can be exported from table and result grids; full streamed export uses the PostgreSQL cursor path.

Copying a table to another connection (`db/table_copy.rs`) maps column types through a canonical type set to the target family, optionally creates the primary key and indexes, and streams rows from the source in 1,000-row pages through the import batch writer. Modes are create, truncate and append. Any failure rolls back the inserted rows and drops a table created by the same run. Read-only targets are rejected. Numeric precision is not available from every catalog and falls back to a wide DECIMAL.
