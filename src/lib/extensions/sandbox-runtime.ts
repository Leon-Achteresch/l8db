...
interface Sandbox {
  ...
  port: MessagePort;
  requests: Map<number, { resolve(value: Json | void): void; reject(error: Error): void; timer: ReturnType<typeof setTimeout> }>;
...
export class SandboxRuntime implements ExtensionRuntime {
  ...
  async load(extension: ExtensionDescriptor, rpc: RpcHandler, failure: (error: Error) => void) {
    ...
    const sandbox: Sandbox = { frame, port: channel.port1, requests: new Map(), failure, sequence: 0 };
    ...
    sandbox.port.onmessage = ({ data: message }) => {
      ...
      if (++count > 500) { failure(new Error("Extension exceeded RPC rate limit")); return }
      if (!message || typeof message !== "object") return;
      if (message.type === "crash") { failure(new Error(String(message.error))); return }
      ...
      if (message.type === "rpc" && Number.isSafeInteger(message.id) && typeof message.method === "string") {
        void rpc(message.method, message.args).then(value => {
          if (this.sandboxes.get(id) === sandbox) sandbox.port.postMessage({ type: "rpc-result", id: message.id, value });
        }, error => {
          if (this.sandboxes.get(id) === sandbox) sandbox.port.postMessage({ type: "rpc-result", id: message.id, error: String(error) });
        });
      }
    };
    const loaded = new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Extension sandbox startup timed out")), this.timeout);
      frame.onload = () => {
        clearTimeout(timer);
    ...
    try {
      await loaded;
      await this.request(id, "load", { code: extension.archive.files[extension.archive.manifest.main], context: { extensionId: id, extensionPath: `extension://${id}/`, storagePath: `extension-storage://${id}/` } });
    } catch (error) { await this.unload(id); throw error }
  }
  private request(id: string, method: string, data: Record<string, unknown> = {}): Promise<Json | void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try { sandbox.port.postMessage({ type: "request", id: requestId, method, ...data }) }
      catch (error) { clearTimeout(timer); sandbox.requests.delete(requestId); reject(error) }
    });
  }
  async activate(id: string) { await this.request(id, "activate") }
  async deactivate(id: string) { await this.request(id, "deactivate") }
  execute(id: string, command: string, payload?: Json) { return this.request(id, "execute", { command, payload }) }
  event(id: string, name: string, payload: Json) { this.sandboxes.get(id)?.port.postMessage({ type: "event", name, payload }) }
  async unload(id: string) {
    const sandbox = this.sandboxes.get(id);
    if (!sandbox) return;
    this.sandboxes.delete(id);
    sandbox.port.postMessage({ type: "terminate" });
    for (const request of sandbox.requests.values()) { clearTimeout(request.timer); request.reject(new Error("Extension runtime stopped")) }
