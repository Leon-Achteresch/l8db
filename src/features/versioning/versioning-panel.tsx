...
export function VersioningPanel() {
  ...
  const fullPage = useRouterState({ select: (state) => state.location.pathname === "/versioning" });
  useEffect(() => {
  ...
  return (
    <motion.aside
      ...
      inert={!open}
      initial={false}
      animate={{ width: open ? (fullPage ? "100%" : 560) : 0, opacity: open ? 1 : 0 }}
      ...
      <div
        className={
          fullPage
            ...
            : "h-full w-[560px] max-w-full border-l border-border/60"
        }
      >
        {(open || visited) && (
          <Suspense
