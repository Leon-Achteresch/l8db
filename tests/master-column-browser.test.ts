...
test.skipIf(!process.env.L8DB_MASTER_DETAIL_BROWSER_URL)(
  "SQL modal binds :master.id across cell changes",
  async () => {
    ...
    try {
      ...
      await page.addInitScript(() => {
        localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              tabsByConnection: {
                perf: [{ kind: "table", schema: "public", table: "table_0000" }],
              },
            },
            version: 4,
          }),
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            state: {
              byConnection: { perf: { panes: ["table:public.table_0000", null], focusedPane: 0 } },
            },
            version: 0,
          }),
        );
        ...
        host.__TAURI_INTERNALS__.invoke = async (command, args) => {
          if (command === "list_foreign_keys")
            return [
              {
                constraint_name: "customer_fk",
                from_schema: "public",
                from_table: "table_0000",
                from_column: "id",
                to_schema: "public",
                to_table: "customers",
          ...
          if (command === "execute_query_with_params")
            return {
              columns: ["matched"],
              rows: [{ matched: `detail-${(args?.params as string[])[0]}` }],
              rows_affected: null,
              execution_time_ms: 1,
            };
      ...
      expect(await dialog.getByRole("button", { name: "Beziehungen", exact: true }).count()).toBe(
        0,
      );
      expect(await dialog.getByLabel("Master-Spalte", { exact: true }).count()).toBe(0);
      ...
      await dialog.getByLabel("FK-/PK-Beziehung").click();
      expect(await page.getByRole("option").count()).toBe(2);
      await page.getByRole("option").nth(1).click();
      await dialog.getByRole("button", { name: "Speichern & anwenden" }).click();
      await page.getByText("detail-3", { exact: true }).waitFor();
      const childSql = await page.evaluate(
        () =>
          Object.values(
