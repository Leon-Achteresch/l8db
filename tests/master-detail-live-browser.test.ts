...
for (const kind of ["postgres", "oracle"] as const) {
  for (const engine of ["chromium", "webkit"] as const) {
    test.skipIf(!bridge)(`${engine}: ${kind} live master-detail SQL from relation to preview and row updates`, async () => {
      ...
      try {
        ...
        for (let attempt = 0; attempt < 45; attempt += 1) {
          try { health = await fetch(`${bridge}/health`).then((response) => response.json()); break; }
          catch { await new Promise((resolve) => setTimeout(resolve, 1000)); }
        }
        if (!health) throw new Error("Master-detail lab bridge did not start");
        await page.exposeFunction("masterDetailLab", async (command: string, args: Record<string, unknown>) => {
          if (command === "list_providers") return health.providers;
          if (command === "execute_query_with_params") calls.push({ sql: String(args.sql), params: args.params as (string | null)[] });
          const delayMs = command === "execute_query_with_params" && (args.params as string[])?.[0] === "102" ? 400 : 0;
          const data = await fetch(bridge!, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind, command, args, delayMs }) }).then((response) => response.json());
          if (data.error) throw new Error(data.error);
          return data.result;
        });
        await seedApp(page, 1);
        await page.addInitScript(({ kind, schema }) => {
          const connections = JSON.parse(localStorage.getItem("l8db.connections") ?? "{}");
          connections.state.connections[0].kind = kind;
          connections.state.connections[0].name = kind === "oracle" ? "Oracle · Test" : "PostgreSQL · Test";
          connections.state.connections[0].connectionString = kind === "oracle" ? "oracle://lab@localhost/FREEPDB1" : "postgresql://lab@localhost/masterdetail";
          localStorage.setItem("l8db.connections", JSON.stringify(connections));
          localStorage.setItem("l8db.table-tabs", JSON.stringify({ state: { tabsByConnection: { perf: [{ kind: "table", schema, table: "MD_ORDERS" }] } }, version: 4 }));
          localStorage.setItem("l8db.split-view", JSON.stringify({ state: { byConnection: { perf: { panes: [`table:${schema}.MD_ORDERS`, null], focusedPane: 0 } } }, version: 0 }));
          const host = window as unknown as { __TAURI_INTERNALS__: { invoke: (command: string, args?: Record<string, unknown>) => Promise<unknown> }; masterDetailLab: (command: string, args: Record<string, unknown>) => Promise<unknown> };
          const original = host.__TAURI_INTERNALS__.invoke;
          host.__TAURI_INTERNALS__.invoke = async (command, args) => {
            if (["list_providers", "list_foreign_keys", "list_table_columns_detailed", "fetch_table_rows", "count_table_rows", "execute_query_with_params"].includes(command)) return host.masterDetailLab(command, args ?? {});
            if (command === "list_schemas") return [schema];
            if (command === "list_tables") return ["MD_ORDERS", "MD_ITEMS", "MD_CUSTOMERS", "MD_TENANTS"].map((name) => ({ schema, name }));
        ...
        await link.click();
        const parentId = await relation.locator("option").filter({ hasText: "MD_CUSTOMERS" }).getAttribute("value");
        ...
        await link.click();
        const compositeId = await relation.locator("option").filter({ hasText: "MD_TENANTS" }).getAttribute("value");
        ...
        await replaceSql(page, editor, `SELECT * FROM "${schema}"."MD_ORDERS" WHERE "REF" = :master.MISSING`);
        await dialog.getByRole("button", { name: "Vorschau", exact: true }).click();
        await preview.getByRole("alert").filter({ hasText: "MISSING" }).waitFor();
        expect(calls.length).toBe(beforeInvalid);
        await replaceSql(page, editor, `SELECT "DOES_NOT_EXIST" FROM "${schema}"."MD_ORDERS" WHERE "REF" = :master.REF`);
        await dialog.getByRole("button", { name: "Vorschau", exact: true }).click();
        await preview.getByRole("alert").filter({ hasText: /DOES_NOT_EXIST|does_not_exist/ }).waitFor();
