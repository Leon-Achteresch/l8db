...
test.skipIf(!process.env.L8DB_QOL_BROWSER)(
  "QoL dialogs: destructive confirmation, paste review, full XLSX worker and task results",
  async () => {
    ...
    try {
      ...
      await page.addInitScript(() => {
        const host = window as unknown as {
          ...
          qolCalls: { command: string; args: unknown }[];
          xlsxBytes: number[];
        };
        ...
        host.__TAURI_INTERNALS__.invoke = async (command, input, options) => {
          ...
          if (command === "list_import_columns")
            return [
              {
                name: "id",
                data_type: "integer",
          ...
          if (command === "read_table_snapshot")
            return {
              columns: ["id", "col_1"],
              rows:
                Number(args.offset ?? 0) === 0
                  ? [
                      { id: 1, col_1: "one" },
                      { id: 2, col_1: "two" },
                      { id: 3, col_1: "third-unloaded-row" },
                    ]
                  : [],
            };
          if (command === "plugin:dialog|save") return "/tmp/qol-browser.xlsx";
          if (command === "plugin:fs|write_file") {
            host.xlsxBytes = Array.from(input as Uint8Array);
            return;
          }
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            state: {
              tabsByConnection: {
                perf: [
                  { kind: "query", id: "qol-ui", title: "QoL", sql: "DELETE FROM table_0000" },
                ],
              },
              recentlyClosed: [],
            },
      ...
      expect(
        await page.evaluate(
          () =>
            (window as unknown as { qolCalls: { command: string }[] }).qolCalls.filter(
              (call) =>
                call.command === "execute_query" || call.command === "execute_in_transaction",
            ).length,
        ),
      ).toBe(0);
      ...
      await page
        .getByRole("heading", { name: "Tabellenblock einfügen", exact: true })
        .waitFor({ state: "hidden" });
      const inserted = await page.evaluate(() =>
        (
          window as unknown as { qolCalls: { command: string; args: Record<string, unknown> }[] }
        ).qolCalls.filter((call) => call.command === "insert_row_in_transaction"),
      );
      ...
      expect(
        await page.evaluate(() =>
          (window as unknown as { qolCalls: { command: string }[] }).qolCalls.some(
            (call) => call.command === "commit_transaction",
          ),
        ),
      ).toBe(false);
      ...
      await page.waitForFunction(
        () => (window as unknown as { xlsxBytes: number[] }).xlsxBytes.length > 0,
      );
      const bytes = await page.evaluate(
        () => (window as unknown as { xlsxBytes: number[] }).xlsxBytes,
      );
      ...
      expect(
        await page.getByText("Tabellenblock einfügen · public.table_0000", { exact: true }).count(),
      ).toBe(1);
      await page.waitForTimeout(500);
      await page.screenshot({ path: "/tmp/l8db-qol-tasks.png" });
      expect(errors).toEqual([]);
    } finally {
      await saveBrowserArtifacts(browser, "qol");
      await browser.close();
      server.stop(true);
    }
  },
